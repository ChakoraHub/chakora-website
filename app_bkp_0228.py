...
def get_db_connection(warehouse='APP_READONLY_WH'):
    """Return fresh Snowflake connection using RSA authentication"""
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        key_path = os.path.join(script_dir, 'rsa_key.p8')

        if not os.path.exists(key_path):
            raise FileNotFoundError("RSA key file not found")
...
def forgot_password():
    ...
    if request.method=="POST":
        ...
        link = f"http:www.chakorahub.com/reset-password/{token}?type={login_type}"
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            # ── Step 1: Verify user was pre-registered by admin ────────────
            # Match by email + phone (no new columns needed)
            cur.execute("""
                SELECT ID
                FROM NRM_USERS
                WHERE EMAIL = %s
            ...
            # ── Step 3: Upsert NRM_LOGINS (no UPDATED_AT column needed) ───
            ...
            if existing_login:
                # Password update — update only password + active status
                cur.execute("""
                    UPDATE NRM_LOGINS
                    SET PASSWORD  = %s,
                        IS_ACTIVE = 'N'
                ...
                # First time — insert new login record
                cur.execute("""
            ...
            # ── Step 5: Update NRM_USERS ───────────────────────────────────
            # UPDATED_AT already exists here — this is where we track
            # when the password was created or last changed
            cur.execute("""
                UPDATE NRM_USERS
                SET USERNAME   = %s,
                    USERTYPE   = 'student',
...
def admin_employee_login():
    """Verify employee credentials before allowing registration"""
    ...
    try:
        ...
        # Check in nrm_users and nrm_logins (main authentication system)
        cursor.execute("""
            SELECT u.ID, u.EMAIL, u.PHONE, u.USERTYPE, l.PASSWORD
            FROM NRM_USERS u
            JOIN NRM_LOGINS l ON u.ID = l.USER_ID
            WHERE (LOWER(u.EMAIL) = LOWER(%s) OR u.PHONE = %s)
            AND l.IS_ACTIVE = 'Y'
        ...
        usertype = user.get('USERTYPE', '').lower()
        if usertype not in ['admin', 'administrator', 'employee']:
            return jsonify({'success': False, 'message': 'Insufficient privileges. Only admin users can register employees.'})

        # Store verification in session (expires in 10 minutes)
...
def admin_register():
    ...
    managers, work_locations = [], []

    # ---------- LOAD ALL DROPDOWNS FROM MASTER TABLES ----------
    if connection:
        cursor = connection.cursor(snowflake.connector.DictCursor)
        ...
        try:
            ...
            # Load employee-related data from MASTER TABLES
            # Departments - Reference Table
            cursor.execute("""
                SELECT DEPT_ID, DEPT_NAME 
                FROM EMP_NRM_DEPARTMENTS 
                ORDER BY DEPT_NAME
            """)
            rows = cursor.fetchall()
            departments = [{"DEPT_ID": r["DEPT_ID"], "DEPT_NAME": r["DEPT_NAME"]} for r in rows]

            # Designations - Reference Table
            cursor.execute("""
                SELECT DESIGNATION_ID, TITLE 
                FROM EMP_NRM_DESIGNATIONS 
                ORDER BY TITLE
            """)
            rows = cursor.fetchall()
            designations = [{"DESIGNATION_ID": r["DESIGNATION_ID"], "TITLE": r["TITLE"]} for r in rows]

            # Managers - Reference Table
            cursor.execute("""
                SELECT MANAGER_ID, MANAGER_NAME 
                FROM EMP_NRM_MANAGERS 
                ORDER BY MANAGER_NAME
            """)
            rows = cursor.fetchall()
            managers = [{"MANAGER_ID": r["MANAGER_ID"], "MANAGER_NAME": r["MANAGER_NAME"]} for r in rows]

            # Work Locations - Reference Table
            cursor.execute("""
                SELECT LOCATION_ID, BRANCH_NAME 
                FROM EMP_NRM_LOCATIONS 
                ORDER BY BRANCH_NAME
            """)
            rows = cursor.fetchall()
            work_locations = [{"LOCATION_ID": r["LOCATION_ID"], "BRANCH_NAME": r["BRANCH_NAME"]} for r in rows]
    ...
    # Handle location field (different field names for student vs employee)
    location_val = data.get("location", "") or data.get("personal_location", "")
    location_val = location_val.strip()

    # Validate registration type
    if registration_type not in ['student', 'employee']:
    ...
    try:
        ...
        cursor.execute("SELECT 1 FROM nrm_users WHERE UPPER(email) = UPPER(%s)", (email,))
        if cursor.fetchone():
        ...
        if cursor.fetchone():
            return jsonify({"success": False, "message": "Phone number already exists in the system."}), 400

        # Generate full name
        full_name = f"{fname} {lname}"
        
        # ========== STUDENT REGISTRATION (NO LOGIN REQUIRED) ==========
        if registration_type == 'student':
            ...
            try:
                ...
                return jsonify({"success": False, "message": "Invalid course or language selection."}), 400
            ...
            cursor.execute("SELECT id FROM nrm_statuses WHERE UPPER(status) = %s LIMIT 1", ('ACTIVE',))
            row = cursor.fetchone()
            if not row:
                return jsonify({"success": False, "message": "Active status not found in database."}), 400
            active_id = row[0]

            # Generate registration ID: {CourseCode}{Initials}{Sequence}{DDMM}
            ...
            cursor.execute("SELECT id FROM nrm_users WHERE UPPER(email) = UPPER(%s) ORDER BY id DESC LIMIT 1", (email,))
            user_id = cursor.fetchone()[0]

            # Step 2: Insert into nrm_students
            cursor.execute("""
                INSERT INTO nrm_students (location, registration_source, user_id)
                VALUES (%s, %s, %s)
            """, (location_val, 'admin', user_id))

            cursor.execute("SELECT id FROM nrm_students WHERE user_id = %s ORDER BY id DESC LIMIT 1", (user_id,))
            student_id = cursor.fetchone()[0]

            # Step 3: Insert into nrm_logins with default password
            ...
            return jsonify({
                "success": True, 
                "message": f"✅ Student registered successfully!\n\nRegistration ID: {reg_id}\nEmail: {email}\nDefault Password: changeme123\n\nPlease ask the student to change their password after first login."
            }), 200

        # ========== EMPLOYEE REGISTRATION (LOGIN REQUIRED) ==========
        else:
            # Check if admin is verified in session
            if not session.get('admin_verified'):
                return jsonify({
                    "success": False, 
                    "message": "⚠️ Authentication required. Please login first to register employees."
                }), 401

            # Check if verification is still valid (within last 10 minutes)
            verified_at_str = session.get('verified_at')
            if verified_at_str:
                verified_at = datetime.fromisoformat(verified_at_str)
                if (datetime.now() - verified_at).total_seconds() > 600:  # 10 minutes
            ...
            # Get employee-specific fields
            department_id = data.get("department", "").strip()
            designation_id = data.get("designation", "").strip()
            ...
            total_experience = data.get("total_experience", "").strip()
            
            # Previous employer 1
            employer1_company = data.get("employer1_company", "").strip()
            employer1_designation = data.get("employer1_designation", "").strip()
            employer1_from = data.get("employer1_from", "").strip()
            employer1_to = data.get("employer1_to", "").strip()
            
            # Previous employer 2
            employer2_company = data.get("employer2_company", "").strip()
            employer2_designation = data.get("employer2_designation", "").strip()
            employer2_from = data.get("employer2_from", "").strip()
            employer2_to = data.get("employer2_to", "").strip()
            
            skills = data.get("skills", "").strip()
            manager_id = data.get("manager", "").strip()
            work_location_id = data.get("work_location", "").strip()
            salary = data.get("salary", "").strip()
            joining_date = data.get("joining_date", "").strip()
            employment_type = data.get("employment_type", "").strip()

            # Validate employee-specific required fields
            if not department_id:
                return jsonify({"success": False, "message": "Department is required."}), 400

            if not designation_id:
                return jsonify({"success": False, "message": "Designation is required."}), 400

            if not dob_raw:
                return jsonify({"success": False, "message": "Date of birth is required."}), 400

            if not experience_level:
                return jsonify({"success": False, "message": "Experience level is required."}), 400
            ...
            if gender and gender not in valid_genders:
                return jsonify({"success": False, "message": "Invalid gender value. Must be Male, Female, or Other."}), 400

            # Validate experience level
            valid_experience_levels = ['Fresher', 'Mid-Level', 'Senior-Level']
            if experience_level not in valid_experience_levels:
                return jsonify({"success": False, "message": "Invalid experience level. Must be Fresher, Mid-Level, or Senior-Level."}), 400

            # FIXED: Validate employment type - case-insensitive comparison with capitalized values
            valid_employment_types = ['Full-Time', 'Part-Time', 'Contract', 'Intern']
            if employment_type:
                # Normalize the employment type by capitalizing each word
                normalized_employment_type = '-'.join(word.capitalize() for word in employment_type.split('-'))
                
                if normalized_employment_type not in valid_employment_types:
                    return jsonify({
                        "success": False, 
                        "message": f"Invalid employment type '{employment_type}'. Must be one of: Full-Time, Part-Time, Contract, or Intern."
                    }), 400
            ...
            try:
                ...
                if age < 18:
                    return jsonify({"success": False, "message": "Employee must be at least 18 years old."}), 400
            except ValueError:
            ...
            if emergency_contact_phone and (not emergency_contact_phone.isdigit() or len(emergency_contact_phone) != 10):
                return jsonify({"success": False, "message": "Emergency contact phone must be exactly 10 digits."}), 400

            # VALIDATE FOREIGN KEY REFERENCES BEFORE INSERT (even without FK constraints)
            # This ensures data integrity at application level
            
            # Validate Department ID exists
            cursor.execute("SELECT DEPT_NAME FROM EMP_NRM_DEPARTMENTS WHERE DEPT_ID = %s", (department_id,))
            dept_row = cursor.fetchone()
            if not dept_row:
                return jsonify({"success": False, "message": f"Invalid department ID: {department_id}. Department not found."}), 400
            department_name = dept_row[0]

            # Validate Designation ID exists
            cursor.execute("SELECT TITLE FROM EMP_NRM_DESIGNATIONS WHERE DESIGNATION_ID = %s", (designation_id,))
            desig_row = cursor.fetchone()
            if not desig_row:
                return jsonify({"success": False, "message": f"Invalid designation ID: {designation_id}. Designation not found."}), 400
            designation_title = desig_row[0]

            # Validate Manager ID if provided
            manager_name = None
            if manager_id:
                cursor.execute("SELECT MANAGER_NAME FROM EMP_NRM_MANAGERS WHERE MANAGER_ID = %s", (manager_id,))
                mgr_row = cursor.fetchone()
                if not mgr_row:
                    return jsonify({"success": False, "message": f"Invalid manager ID: {manager_id}. Manager not found."}), 400
                manager_name = mgr_row[0]

            # Validate Work Location ID if provided
            work_location_name = None
            if work_location_id:
                cursor.execute("SELECT BRANCH_NAME FROM EMP_NRM_LOCATIONS WHERE LOCATION_ID = %s", (work_location_id,))
                loc_row = cursor.fetchone()
                if not loc_row:
                    return jsonify({"success": False, "message": f"Invalid work location ID: {work_location_id}. Location not found."}), 400
                work_location_name = loc_row[0]

            # Generate employee ID: EMP-{Designation}-{First3Letters}-{Sequence}
            first_three = fname[:3].upper() if len(fname) >= 3 else fname.upper().ljust(3, 'X')
            
            # Get the next sequence number for this designation and first name combination
            cursor.execute("""
                SELECT COUNT(*) FROM EMPLOYEE_REGISTRATIONS 
                WHERE EMPLOYEE_ID LIKE %s
            """, (f'EMP-{designation_id}-{first_three}-%',))
            
            count_result = cursor.fetchone()
            seq_number = (count_result[0] if count_result else 0) + 1
                
            employee_id = f"EMP-{designation_id}-{first_three}-{str(seq_number).zfill(3)}"

            # Generate application ID
            application_id = f"APP{employee_id.replace('-', '')}"

            # Step 1: Insert into nrm_users for authentication
            cursor.execute("""
                INSERT INTO nrm_users (username, email, phone, profile_pic, usertype, registration_source)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (full_name, email, phone, 'default.jpg', 'employee', 'admin'))
            
            cursor.execute("SELECT id FROM nrm_users WHERE UPPER(email) = UPPER(%s) ORDER BY id DESC LIMIT 1", (email,))
            user_id = cursor.fetchone()[0]

            # Step 2: Insert into nrm_logins with default password
            hashed_pwd = generate_password_hash("changeme123")
            cursor.execute("""
                INSERT INTO nrm_logins (user_id, password, is_active)
                VALUES (%s, %s, %s)
            ...
            # Step 3: Insert into EMPLOYEE_REGISTRATIONS table
            # Set timestamps manually
            current_time = datetime.now()
            
            try:
                # ✅ UPDATED: All ID fields are now VARCHAR(50) - no type conversion needed
                cursor.execute("""
                    INSERT INTO EMPLOYEE_REGISTRATIONS 
                    (EMPLOYEE_ID, APPLICATION_ID, FIRST_NAME, LAST_NAME, FULL_NAME, 
                     EMAIL, PHONE, DATE_OF_BIRTH, GENDER, ADDRESS,
                     EMERGENCY_CONTACT_NAME, EMERGENCY_CONTACT_PHONE,
                     EXPERIENCE_LEVEL, TOTAL_EXPERIENCE,
                     EMPLOYER1_COMPANY, EMPLOYER1_DESIGNATION, EMPLOYER1_FROM, EMPLOYER1_TO,
                     EMPLOYER2_COMPANY, EMPLOYER2_DESIGNATION, EMPLOYER2_FROM, EMPLOYER2_TO,
                     SKILLS, DEPARTMENT_ID, DEPARTMENT_NAME,
                     DESIGNATION_ID, DESIGNATION_TITLE, MANAGER_ID, MANAGER_NAME,
                     WORK_LOCATION_ID, WORK_LOCATION_NAME, PERSONAL_LOCATION, 
                     SALARY, JOINING_DATE, EMPLOYMENT_TYPE,
                     STATUS, USER_ID, PASSWORD_HASH, CREATED_BY, CREATED_AT, UPDATED_AT)
                ...
                """, (
                    ...
                    phone,                          # VARCHAR(15)
                    dob_date,                       # DATE
                    gender if gender else None,     # VARCHAR(10)
                    address if address else None,   # TEXT
                    emergency_contact_name if emergency_contact_name else None,  # VARCHAR(200)
                    emergency_contact_phone if emergency_contact_phone else None, # VARCHAR(15)
                    experience_level,               # VARCHAR(20)
                    total_experience if total_experience else None,  # VARCHAR(50)
                    employer1_company if employer1_company else None,         # VARCHAR(200)
                    employer1_designation if employer1_designation else None, # VARCHAR(100)
                    employer1_from if employer1_from else None,               # VARCHAR(20)
                    employer1_to if employer1_to else None,                   # VARCHAR(20)
                    employer2_company if employer2_company else None,         # VARCHAR(200)
                    employer2_designation if employer2_designation else None, # VARCHAR(100)
                    employer2_from if employer2_from else None,               # VARCHAR(20)
                    employer2_to if employer2_to else None,                   # VARCHAR(20)
                    skills if skills else None,     # TEXT
                    department_id,                  # VARCHAR(50) - ✅ No conversion needed
                    department_name,                # VARCHAR(200)
                    designation_id,                 # VARCHAR(50) - ✅ No conversion needed
                    designation_title,              # VARCHAR(100)
                    manager_id if manager_id else None,        # VARCHAR(50) - ✅ No conversion needed
                    manager_name if manager_name else None,    # VARCHAR(200)
                    work_location_id if work_location_id else None,     # VARCHAR(50) - ✅ No conversion needed
                    work_location_name if work_location_name else None, # VARCHAR(200)
                    location_val,                   # VARCHAR(200)
                    salary if salary else None,     # VARCHAR(50)
                    joining_date if joining_date else None,  # DATE
                    employment_type if employment_type else None,  # VARCHAR(20)
                    'ACTIVE',                       # VARCHAR(20)
                    user_id,                        # INTEGER
                    hashed_pwd,                     # VARCHAR(255)
                    session.get('verified_user_id', 'ADMIN'),  # VARCHAR(100)
                    current_time,                   # TIMESTAMP_LTZ
                    current_time                    # TIMESTAMP_LTZ
                ))
            except Exception as insert_error:
                error_msg = str(insert_error)
                print(f"Employee registration insert error: {error_msg}")
                import traceback
                traceback.print_exc()
                ...
                if "invalid identifier" in error_msg.lower():
                    return jsonify({
                        "success": False, 
                        "message": f"Database schema error: Column name mismatch. Please contact support."
                    }), 500
                elif "invalid number" in error_msg.lower():
                    return jsonify({
                        "success": False, 
                        "message": f"Data type error: Please verify all numeric fields are valid."
                    }), 500
                else:
                    return jsonify({
                        "success": False, 
                        "message": f"Failed to save employee registration: {error_msg}"
                    }), 500
            ...
            return jsonify({
                ...
                "message": f"✅ Employee registered successfully!\n\n" + 
                          ...
                          f"Designation: {designation_title}\n" +
                          f"Email: {email}\n" +
                          f"Default Password: changeme123\n\n" +
                          f"Please ask the employee to change their password after first login."
            }), 200

    except Exception as e:
        if connection:
            connection.rollback()
        print(f"ERROR in admin_register: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({
            "success": False, 
...
def submit_nrm_enquiries():
    ...
    try:
        ...
        if not conn:
            ...
            print(f"❌ DB Connection failed in submit_nrm_enquiries")
            if is_ajax:
