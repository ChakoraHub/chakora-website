...
function renderStartScreen() {
    ...
    const description = isMock
        ? "Practice mode: instant feedback shown after each answer. Your webcam will be recorded. No proctoring or violations tracked."
        : "By clicking start, you will enter a secure fullscreen mode. Webcam access is required. All activity is monitored.";
...
function requestWebcamThenStart() {
    ...
    webcamPromise
        ...
        .then(() => {
            // CERTIFICATION: enforce fullscreen
            // MOCK_TEST: skip fullscreen, start directly
            const isMock = (typeof EXAM_TYPE !== "undefined" && EXAM_TYPE === "MOCK_TEST");
            if (isMock) {
                startExamAttempt();
            } else {
...
function requestFullscreenAndStart() {
    ...
    if (requestFS) {
        requestFS.call(docEl)
            ...
            .catch(() => {
                alert("Security policy requires full screen to continue. Please click start again and allow fullscreen.");
                renderStartScreen();
            });
    } else {
        startExamAttempt();
...
function startExamAttempt() {
    ...
    .then(data => {
        ...
        // CERTIFICATION only: activate proctoring + violation tracking
        // MOCK_TEST: webcam recording starts (via proctor.js) but no violation hooks
        if (operationalMode === "CERTIFICATION") {
            if (typeof initProctoring === "function") {
                initProctoring(window.attemptId);
            }
        } else {
            // MOCK_TEST: start webcam recording only (no violation listeners)
            if (typeof startWebcamRecordingOnly === "function") {
                startWebcamRecordingOnly(window.attemptId);
            }
        }
...
function renderQuestion() {
    ...
    const optionsHTML   = optionLetters.map((letter, i) => `
        <div class="option-box ${selected === letter ? 'selected' : ''} ${optionClass(letter)}"
             ...
             ${(isMock && alreadyAnswered) ? 'style="pointer-events:none;"' : ''}>
...
function selectOption(option, el) {
    ...
    // MOCK_TEST: lock in answer — cannot change once selected
    if (isMock && userAnswers[q.id] !== null) return;
